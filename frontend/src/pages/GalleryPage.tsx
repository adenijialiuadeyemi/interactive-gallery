...
export default function GalleryPage() {
  ...
  const [mode, setMode] = useState<'scroll' | 'pagination'>(
    window.innerWidth < 768 ? 'scroll' : 'pagination'
  );

  useEffect(() => {
    const handleResize = () => {
      setMode(window.innerWidth < 768 ? 'scroll' : 'pagination');
    };
  ...
  useEffect(() => {
    const loadImages = async () => {
      try {
        const res = await axios.get(`/images/unsplash?page=${page}&perPage=12`);
